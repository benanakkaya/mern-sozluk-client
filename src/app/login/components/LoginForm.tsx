...
const LoginForm = () => {
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values) => {
      ...
      axios.post('http://localhost:5000/user/login', values).then(async (res) => {
    ...
    validationSchema: yup.object().shape({
      email: yup.string().email("Lütfen geçerli bir e-mail gir!").required('e-mailin olmadan nasıl giriş yapacaksın?'),
  ...
  return (
    <>
      <form onSubmit={formik.handleSubmit} className="flex flex-col gap-4">
        ...
        <button disabled={loginStatus === "pending"} type="submit" className="bg-primary text-white rounded-md px-2 py-1 flex items-center justify-center">
            {loginStatus === "pending" ? (
